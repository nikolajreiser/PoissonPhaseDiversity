...
from src.image_functions import scl_imgs, defocus, save_data, progress, errA, ift, dcts, get_H2
...
error_im_perf = np.zeros((len(noise_vals), num_points))
noise_imgs = np.zeros((len(noise_vals), imsize, imsize))
...
for i, noise_val in enumerate(noise_vals):
    for j in range(num_points):
        ...
        if j == 0: noise_imgs[i] = imgs[-1]
        

save_data('sweep_deconv', noise_imgs, error_decon, error_im_aber, error_im_corr, error_im_perf, noise_vals, errs_p)